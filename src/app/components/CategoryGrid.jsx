      ...
      <section className="w-full bg-gradient-to-br from-gray-50 via-blue-50/30 to-purple-50/30 py-16">
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white/60 backdrop-blur-sm rounded-3xl shadow-lg border border-white/20 p-8">
            {/* Mobile view - 2-row grid that scrolls horizontally */}
    ...
    <section className="w-full bg-white via-blue-50/30 to-purple-50/30 py-10">
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-[#f9fafb] backdrop-blur-sm rounded-3xl shadow-lg border border-white/20 p-6">
          {/* Mobile view - 2-row grid that scrolls horizontally */}
          <div className="sm:hidden overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory">
            <motion.div 
              className="grid grid-rows-2 grid-flow-col gap-3 min-w-min"
              variants={container}
              initial="hidden"
              animate="show"
            >
              {categories.map((category) => (
                <motion.div
                  key={category._id}
                  className="w-[150px] snap-start cursor-pointer group"
                  variants={item}
                  ...
                  <div className="relative overflow-hidden rounded-xl bg-white shadow-sm hover:shadow-lg transition-all duration-300 group-hover:shadow-blue-500/20">
                    <div className="relative aspect-square overflow-hidden">
                      <img
                        ...
                        sizes="(max-width: 640px) 150px, (max-width: 768px) 33vw, (max-width: 1024px) 25vw, (max-width: 1280px) 20vw, 16vw"
                        priority
                      />
          ...
          {/* Tablet and desktop view */}
          <motion.div 
            className="hidden sm:grid sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 pb-4"
            variants={container}
            initial="hidden"
            animate="show"
          >
